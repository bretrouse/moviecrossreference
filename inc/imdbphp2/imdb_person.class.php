  ...
  public function name() {
    if (empty($this->fullname)) {
      ...
      if (preg_match("/<title>(.*?)<\/title>/i",$this->page["Name"],$match)) {
        $this->fullname = str_replace(' - IMDb','',trim($match[1]));
      }
    }
  ...
  public function results($url="") {
   ...
   for ($i=0;$i<$mc;++$i) {
     if ($i == $maxresults) break; // limit result count	
 	 if( preg_match("/media\_strip/", $matches[0][$i]) && !preg_match("/Popular Names/", $matches[0][$i])  && !preg_match("/Exact Matches/", $matches[0][$i]) ) continue;
     $pid = $matches[1][$i];
     ...
     if (!empty($info)) {
       if (preg_match('|\s*\((.*)\)\s*<small>\((.*),\s*<a href="/title/tt(\d{7})/">(.*)</a>\s*\((\d{4})\)\)|Ui',$info,$match)) {
		 $roman = $match[1];
         $role = $match[2];
         $mid  = $match[3];
         $movie= $match[4];
	 ...
	 if( preg_match("/Popular Names/", $matches[0][$i]) )
		$tmpres->pop = true;
     $this->resu[$i] = $tmpres;
     unset($tmpres);
   }
